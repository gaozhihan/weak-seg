    ...
    for phase in ['train', 'val']:
        if phase == 'train':
            ...
            for data in dataloader.dataloaders["train"]:
                ...
                if flag_use_cuda:
                    arg_paral = [(labels[i,:].cpu().numpy(), mask_gt[i,:,:].numpy(), mask[i,:,:,:].detach().numpy(), img[i,:,:,:].numpy(), preds1[i,:].detach().cpu().numpy(), args.preds_only) for i in range(labels.shape[0])]
                else:
                    arg_paral = [(labels[i,:].numpy(), mask_gt[i,:,:].numpy(), mask[i,:,:,:].detach().numpy(), img[i,:,:,:].numpy(), preds1[i,:].detach().numpy(), args.preds_only) for i in range(labels.shape[0])]
                temp = my_pool.starmap(crf.runCRF, arg_paral)

                for i in range(labels.shape[0]):
                    mask_s_gt_np[i,:,:,:] = temp[i][0]
                    confidence[i] = temp[i][2]
                    mask_seed[i,:,:,:] = temp[i][3]
