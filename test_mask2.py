...
with torch.no_grad():
        ...
        if phase == 'train':
            ...
            for data in dataloader.dataloaders["train"]:
                ...
                for i in range(labels.shape[0]):
                    if flag_use_cuda:
                        mask_s_gt_np[i,:,:,:], mask_pred = crf.runCRF(labels[i,:].cpu().numpy(), mask_gt[i,:,:].numpy(), mask[i,:,:,:].detach().numpy(), img[i,:,:,:].numpy(), preds1[i,:].detach().cpu().numpy(), args.preds_only)
                    else:
                        mask_s_gt_np[i,:,:,:], mask_pred = crf.runCRF(labels[i,:].numpy(), mask_gt[i,:,:].numpy(), mask[i,:,:,:].detach().numpy(), img[i,:,:,:].numpy(), preds1[i,:].detach().numpy(), args.preds_only)
            ...
            for data in dataloader.dataloaders["val"]:
                ...
                for i in range(labels.shape[0]):
                    if flag_use_cuda:
                        mask_s_gt_np[i,:,:,:], mask_pred = crf.runCRF(preds1[i,:].cpu().numpy(), mask_gt[i,:,:].numpy(), mask[i,:,:,:].detach().numpy(), img[i,:,:,:].numpy(), preds1[i,:].detach().cpu().numpy(), args.preds_only)
                    else:
                        mask_s_gt_np[i,:,:,:], mask_pred = crf.runCRF(preds1[i,:].numpy(), mask_gt[i,:,:].numpy(), mask[i,:,:,:].detach().numpy(), img[i,:,:,:].numpy(), preds1[i,:].detach().numpy(), args.preds_only)
    ...
    print('1 It took {:.2f}, Train Loss: {:.4f}, Acc: {:.4f}, Recall: {:.4f}; eval loss: {:.4f}, Acc: {:.4f}, Recall: {:.4f}'.format(time_took, epoch_train_loss1, acc_train1, recall_train1, epoch_eval_loss1, acc_eval1, recall_eval1))
    print('2 It took {:.2f}, Train Loss: {:.4f}, Acc: {:.4f}, Recall: {:.4f}; eval loss: {:.4f}, Acc: {:.4f}, Recall: {:.4f}'.format(time_took, epoch_train_loss2, acc_train2, recall_train2, epoch_eval_loss2, acc_eval2, recall_eval2))
