...
class VOCData():
    def __init__(self, args, cues_pickle_dir1, cues_pickle_dir2, cues_pickle_snap_dir1, cues_pickle_snap_dir2):
        if args.model ==  "SEC":
        ...
        self.image_datasets = {x:VOCDataset(x+".txt", args, cues_pickle_dir1, cues_pickle_dir2, cues_pickle_snap_dir1, cues_pickle_snap_dir2, self.data_transforms[x])
                          for x in ['train', 'val']}
...
class VOCDataset(Dataset):
    ...
    def __init__(self, list_file,  args, cues_pickle_dir1, cues_pickle_dir2, cues_pickle_snap_dir1, cues_pickle_snap_dir2, transform=None):
        self.args = args
        ...
        self.cues_snap_data1 = pickle.load(open(cues_pickle_snap_dir1,'rb'))
        self.cues_snap_data2 = pickle.load(open(cues_pickle_snap_dir2,'rb'))
    ...
    def __get_cues_from_img_name(self, img_name):
        ...
        cues1 = self.cues_data1['%i_cues' % img_id_sec]
        ...
        cues2 = self.cues_data2['%i_cues' % img_id_sec]
        ...
        cues1_snap = self.cues_snap_data1['%i_cues' % img_id_sec]
        cues_snap_numpy1 = np.zeros([self.args.num_classes, self.args.output_size[0], self.args.output_size[1]])
        cues_snap_numpy1[cues1_snap[0], cues1_snap[1], cues1_snap[2]] = 1.0

        cues2_snap = self.cues_snap_data2['%i_cues' % img_id_sec]
        cues_snap_numpy2 = np.zeros([self.args.num_classes, self.args.output_size[0], self.args.output_size[1]])
        cues_snap_numpy2[cues2_snap[0], cues2_snap[1], cues2_snap[2]] = 1.0

        return  cues_numpy1.astype('float32'), cues_numpy2.astype('float32'), cues_snap_numpy1.astype('float32'), cues_snap_numpy2.astype('float32')
    ...
    def __getitem__(self, idx):
        ...
        img_gray = Image.fromarray(temp_rgb,mode='RGB')

        if self.args.origin_size:
            img_array = np.array(img).astype(np.float32)
            img_gray_array = np.array(img_gray).astype(np.float32)
            mask = np.array(Image.open(mask_name))
        else:
            img_array = np.array(img.resize(self.size)).astype(np.float32)
            img_gray_array = np.array(img_gray.resize(self.size)).astype(np.float32)
        ...
        if self.file_list[idx]+".png" in self.img_id_dic_SEC.keys():
            cues_numpy1, cues_numpy2, cues_snap_numpy1, cues_snap_numpy2 = self.__get_cues_from_img_name(self.file_list[idx]+".png")
            cues_merge_numpy = np.maximum(cues_snap_numpy1, cues_snap_numpy2)
            return label_ts, mask, img_array, img_gray_array, cues_numpy1, cues_numpy2, cues_snap_numpy1, cues_snap_numpy2, cues_merge_numpy, super_pixel.astype('int32') #self.file_list[idx]  # img_name,

        else:
            # return img_ts, label_ts, mask, img_name, img_array
            return label_ts, mask, img_array, img_gray_array #self.file_list[idx]
...
if __name__ == '__main__':
    ...
    cues_pickle_dir1 = "/home/sunting/Documents/program/pyTorch/weak_seg/st_resnet/models/st_resnet_cue_01.pickle"
    ...
    cues_pickle_dir2 = "/home/sunting/Documents/program/pyTorch/weak_seg/st_resnet/models/st_resnet_cue_01_mul_scal_pure_gray.pickle"


    cues_pickle_snap_dir1 = "/home/sunting/Documents/program/pyTorch/weak_seg/st_resnet/models/st_resnet_cue_01_hard_snapped.pickle"
    cues_pickle_snap_dir2 = "/home/sunting/Documents/program/pyTorch/weak_seg/st_resnet/models/st_resnet_cue_01_mul_scal_pure_gray_hard_snapped.pickle"


    dataloader = VOCData(args, cues_pickle_dir1, cues_pickle_dir2, cues_pickle_snap_dir1, cues_pickle_snap_dir2)
    ...
    with torch.no_grad():
        ...
        for data in dataloader.dataloaders["train"]:
            labels, mask_gt, img, img_gray, cues1, cues2, cues_snap1, cues_snap2, cues_merge, super_pixel = data

            # fist line
            plt.subplot(3,5,1); plt.imshow(img[0]/255); plt.title('image'); plt.axis('off')
            plt.subplot(3,5,2); plt.imshow(img_gray[0]/255); plt.title('gray image'); plt.axis('off')
            temp = mask_gt[0,:,:].numpy()
            temp[temp==255] = 0
            plt.subplot(3,5,3); plt.imshow(temp); plt.title('gt'); plt.axis('off')
            plt.subplot(3,5,4); plt.imshow(super_pixel[0]); plt.title('super pixel'); plt.axis('off')

            # cue1
            temp1 = cues1.detach().squeeze().numpy()
            plt.subplot(3,5,6); plt.imshow(np.argmax(temp1,axis=0)); plt.title('cues1'); plt.axis('off')
            plt.subplot(3,5,7); plt.imshow(temp1[0,:,:]); plt.title('bk cues1'); plt.axis('off')

            temp1 = cues_snap1.detach().squeeze().numpy()
            plt.subplot(3,5,8); plt.imshow(np.argmax(temp1,axis=0)); plt.title('cues1 snap'); plt.axis('off')
            plt.subplot(3,5,9); plt.imshow(temp1[0,:,:]); plt.title('bk cues1 snap'); plt.axis('off')

            # cue2
            temp1 = cues2.detach().squeeze().numpy()
            plt.subplot(3,5,11); plt.imshow(np.argmax(temp1,axis=0)); plt.title('cues2'); plt.axis('off')
            plt.subplot(3,5,12); plt.imshow(temp1[0,:,:]); plt.title('bk cues2'); plt.axis('off')

            temp1 = cues_snap2.detach().squeeze().numpy()
            plt.subplot(3,5,13); plt.imshow(np.argmax(temp1,axis=0)); plt.title('cues2 snap'); plt.axis('off')
            plt.subplot(3,5,14); plt.imshow(temp1[0,:,:]); plt.title('bk cues2 snap'); plt.axis('off')

            # merged
            temp1 = cues_merge.detach().squeeze().numpy()
            plt.subplot(3,5,10); plt.imshow(np.argmax(temp1,axis=0)); plt.title('merged'); plt.axis('off')
            plt.subplot(3,5,15); plt.imshow(temp1[0,:,:]); plt.title('bk merged'); plt.axis('off')
