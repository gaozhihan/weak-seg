...
class CRFLayer():
    ...
    def run(self, sm_mask, img, flag_train): # the input array are detached numpy already
        batch_size = sm_mask.shape[0]
        result = np.zeros(sm_mask.shape)
        for i in range(batch_size):

            sm_u = np.transpose(resize(np.transpose(sm_mask[i], [1,2,0]), self.input_size, mode='constant'),[3,0,1])
    ...
    def run_parallel(self, sm_mask, img, flag_train): # flag_train is for the strange dif between train & test in org SEC
        batch_size = sm_mask.shape[0]
        unary = np.transpose(sm_mask, [0, 2, 3, 1])
