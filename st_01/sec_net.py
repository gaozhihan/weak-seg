...
def crf(sm_mask_one, img_one, num_class, input_size, mask_size, num_iter):
    ...
    return np.transpose(resize(np.transpose(np.array(Q).reshape((num_class, input_size[0], input_size[1])),[1,2,0]), mask_size, mode='constant'), [2,0,1])
...
class CRFLayer():
    ...
    def run(self, sm_mask, img): # the input array are detached numpy already
        ...
        for i in range(batch_size):
            ...
            result[i] = np.transpose(resize(np.transpose(np.array(Q).reshape((self.num_class, self.input_size[0], self.input_size[1])),[1,2,0]), self.mask_size, mode='constant'), [2,0,1])

        return result

    # for parallel running ------------------------------
    def crf(self, sm_mask_one, img_one):
        sm_u = np.transpose(resize(np.transpose(sm_mask_one, [1,2,0]), self.input_size, mode='constant'),[2,0,1])
        ...
        Q = d.inference(self.nun_iter)
        return np.transpose(resize(np.transpose(np.array(Q).reshape((self.num_class, self.input_size[0], self.input_size[1])),[1,2,0]), self.mask_size, mode='constant'), [2,0,1])
