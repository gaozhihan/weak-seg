...
import multi_scale.voc_data_mul_scale_w_cues_rgb
...
dataloader = multi_scale.voc_data_mul_scale_w_cues_rgb.VOCData(args)
...
main_scheduler = optim.lr_scheduler.StepLR(optimizer, step_size=3, gamma=0.4)
...
for epoch in range(args.epochs):
    ...
    for data in dataloader.dataloaders["train"]:
        inputs, labels, mask_gt, img, cues, img_array_color = data
        ...
        img_np_temp = img_array_color.detach().numpy()/255.0
    ...
    for data in dataloader.dataloaders["val"]:
        ...
        with torch.no_grad():
            ...
            if args.CRF_model == 'adaptive_CRF':
                result_big, result_small = st_crf_layer.run(sm_mask.detach().cpu().numpy(), img_array_color.numpy(), labels.detach().cpu().numpy())
            else:
                result_big, result_small = st_crf_layer.run(sm_mask.detach().cpu().numpy(), img_array_color.numpy())
