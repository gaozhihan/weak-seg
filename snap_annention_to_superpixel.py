...
def snap_anntention_to_superpixel(outputs, img, mask_gt, flag_classify, labels, super_pixel_path):
    ...
    with torch.no_grad():
        ...
        plt.subplot(3,(2 + num_cur_class),1); plt.imshow(img); plt.title('Input image'); plt.axis('off')
        plt.subplot(3,(2 + num_cur_class),3 + num_cur_class); plt.imshow(mask_gt); plt.title('true mask'); plt.axis('off')
        plt.subplot(3,(2 + num_cur_class),2); plt.imshow(super_pixel); plt.title('super pixel'); plt.axis('off')
        plt.subplot(3,(2 + num_cur_class),4 + num_cur_class); plt.imshow(mark_boundaries(img,super_pixel)); plt.title('boundary'); plt.axis('off')

        for i in range(num_cur_class):
            plt.subplot(3,(2 + num_cur_class),3+i); plt.imshow(attention[cur_class[i],:,:]); plt.title('raw attention {}'.format(cur_class[i])); plt.axis('off')
            snapped_attention = snap_to_superpixel(attention[cur_class[i],:,:], img, super_pixel)
            plt.subplot(3,(2 + num_cur_class),num_cur_class+5+i); plt.imshow(snapped_attention); plt.title('snapped attention {}'.format(cur_class[i])); plt.axis('off')
            thr = snapped_attention.max() * thr_ratio
            seed = (snapped_attention > thr)
            plt.subplot(3,(2 + num_cur_class),num_cur_class*2+7+i); plt.imshow(seed); plt.title('seed {}'.format(cur_class[i])); plt.axis('off')
        ...
        for phase in ['train', 'val']:
            if phase == 'train':
                ...
                for data in dataloader.dataloaders["train"]:
                    ...
                    snap_anntention_to_superpixel(outputs, img, mask_gt, flag_classify, labels.detach().squeeze().numpy(), super_pixel_path+img_name[0]+'.npy')
